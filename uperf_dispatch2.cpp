...
void genCodetest(DispatchCase caseId, unsigned char *instBuf, int testCnt, int changePoint, int codeLoopCnt) {
    int i = 0;
#if defined(__aarch64__) || defined(_M_ARM64)
...
int main(int argc, char *argv[]) {
    ...
    for (int testCnt = testBase; testCnt < testEnd; testCnt += testStep) {
        printf("%d, ", testCnt);
        genCodetest(caseId, instBuf, testCnt, changePoint, codeLoopCnt);
        printf("\n");
    }
